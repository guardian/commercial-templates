...
test.describe('CAPI Multiple Paidfor Redesign', () => {
	test('Get reference screenshots', async ({ page }) => {
		await page.setViewportSize(viewport);

		await page.goto(`${referenceBaseUrl}csr/capi-multiple-paidfor-redesign/`, {
		...
		for (const width of templatePreviewWidths) {
			...
			await referenceTemplateLocator.screenshot({
				path: `./playwright/reference-images/CAPI-Multiple-Paidfor-Redesign-${width.replace('%', '')}.png`,
			});
		}
	});
	...
	test('Compare PR templates to reference screenshots', async ({ page }) => {
		...
		await page.goto(`${localBaseUrl}csr/capi-multiple-paidfor-redesign`, {
			waitUntil: 'networkidle',
		});
		...
		for (const width of templatePreviewWidths) {
			...
			await expect(testTemplateLocator).toHaveScreenshot(
				`CAPI-Multiple-Paidfor-Redesign-${width.replace('%', '')}.png`,
