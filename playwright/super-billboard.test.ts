...
test.describe('Super billboard', () => {
	test('Get reference screenshots', async ({ page }) => {
		await page.setViewportSize(viewport);

		await page.goto(`${referenceBaseUrl}/super-billboard/`, {
		...
		for (const width of templatePreviewWidths) {
			...
			referenceTemplateHtmlLocator.locator('button').click();
			...
			await referenceTemplateLocator.screenshot({
				path: `./playwright/reference-images/Super-billboard-${width.replace('%', '')}-expanded.png`,
			});
			...
			await referenceTemplateLocator.screenshot({
				path: `./playwright/reference-images/Super-billboard-${width.replace('%', '')}-not-expanded.png`,
			});
		}
	});
	...
	test('Compare PR templates to reference screenshots', async ({ page }) => {
		...
		await page.goto(`${localBaseUrl}/Super-billboard`, {
			waitUntil: 'networkidle',
		});
		...
		for (const width of templatePreviewWidths) {
			...
			// expand the template ad
			testTemplateHtmlLocator.locator('button').click();
			// wait for template expansion transition
			await new Promise((r) => setTimeout(r, 1500));
			// take a reference screenshot of the expanded template
			await expect(testTemplateLocator).toHaveScreenshot(
				`Super-billboard-${width.replace('%', '')}-expanded.png`,
			...
			//take a reference screenshot of the template when not expanded
			await expect(testTemplateLocator).toHaveScreenshot(
				`Super-billboard-${width.replace('%', '')}-not-expanded.png`,
				{ maxDiffPixelRatio: 0.006 },
			);
		}
	});
