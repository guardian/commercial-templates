...
test.describe('CAPI Single Paidfor Redesign', () => {
	test('Get reference screenshots', async ({ page }) => {
		await page.setViewportSize(viewport);

		await page.goto(`${referenceBaseUrl}csr/capi-single-paidfor-redesign/`, {
		...
		for (const width of templatePreviewWidths) {
			...
			await referenceTemplateLocator.screenshot({
				path: `./playwright/reference-images/CAPI-Single-Paidfor-Redesign-${width.replace('%', '')}.png`,
			});
		}
	});
	...
	test('Compare PR templates to reference screenshots', async ({ page }) => {
		...
		await page.goto(`${localBaseUrl}csr/capi-single-paidfor-redesign`, {
			waitUntil: 'networkidle',
		});
		...
		for (const width of templatePreviewWidths) {
			...
			await expect(testTemplateLocator).toHaveScreenshot(
				`CAPI-Single-Paidfor-Redesign-${width.replace('%', '')}.png`,
